...
| 保存时报「该字段是必填字段」 | 必填项未填 / XPath 未点「为定位器」/ 未关「未找到匹配结果」下拉 | `element-selector.md` C1：close icon → 等 1s → 点建议 → 无红框再保存 |
...
| **指令顺序错误**（如「输入文本」在「打开网页」前） | 在开始节点下方而非末尾追加插入 | **首选**：选中错位节点 → 右击 **剪切** → 选中锚点行 → **Enter** 创建空行 → **粘贴**（`platform-ops.md` §2.3a，保留已填配置）；剪切失败再删后重插 |
