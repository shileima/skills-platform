...
- **先定位插入点**：确定要插入到哪一行**后面**（锚点行）；首条业务指令时点 canvas「拖拽添加指令」提示行让光标进入编辑区；非首条则选中锚点行 → **End** → **Return** 空出一行
...
| 开始 → **输入文本** → **打开网页** → 结束 | ❌ 错误：选中「输入文本」→ 右击 **剪切** → 选中「打开网页」→ 按 **End** → **Return** 创建空行 → **粘贴**（见 `platform-ops.md` §2.3a，不必先删除） |
...
4. **保存前校验**（见 `platform-ops.md` §2.4，**必做、不可跳过**）：
   ...
   - 所有带 `*` 的必填项已填写且**不为空**
   - 弹框内**无任何必填输入框仍带红色边框**
   - 元素选择器：**方式 B（捕获）** 或 **方式 C（Cmd+V 粘贴 XPath → 按 Enter）**——见 `element-selector.md`，禁止其他方式
