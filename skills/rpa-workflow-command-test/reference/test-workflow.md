...
> ⚠️ **追加式插入**：每条新指令必须加在**已有指令链末尾**（最后一条之后、结束节点之前），**禁止**在开始节点后或已有指令之间插队。详见 `insert-command.md` §插入位置约束。
>
> 🚫🚫🚫 **非首条指令强制铁律**：必须先**单击 canvas 中最后一条已保存指令那一行 → 按 Enter 键**，在其**正下方**空出新行，光标落入新空行后才能搜索/双击插入下一条。**绝不能**把光标停在开始节点下方、第一条指令上方、或任意两条已有指令**之间**再插入——否则新指令会排到已有指令**前面**，顺序即刻非法。

操作要点（详见 `insert-command.md`）：

- **先定位插入点**：确定要插入到哪一行**后面**（锚点行）；首条业务指令时点 canvas「拖拽添加指令」提示行让光标进入编辑区；**非首条（必执）**：单击锚点行（最后一条已保存指令）→ 按 **Enter** → 其正下方空出新行 → 全量抓 AX 验证空行已生成
