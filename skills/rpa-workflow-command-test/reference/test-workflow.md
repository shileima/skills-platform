...
在**结束节点上方**依次插入指令，顺序由测试场景定义（如：打开网页 → 输入文本 → 点击）。

> ⚠️ **追加式插入**：每条新指令必须加在**已有指令链末尾**（最后一条之后、结束节点之前），**禁止**在开始节点后或已有指令之间插队。详见 `platform-ops.md` §2.3「插入位置约束」。

操作要点（详见 `platform-ops.md` §2.3）：

- **先定位插入点**：确定要插入到哪一行**后面**（锚点行）；若该行后面**没有空行**，选中该行 → 按 **Enter** 空出一行
- 在该空行 **粘贴**（剪切/复制后），或输入 **`/`** 弹出指令浮层（**不要**在开始节点下方空行插入第 2、3 条指令）
- **在浮层搜索框（「请输入」）里输入中文指令名**，如「打开网页」「输入文本」「点击」
- **点击列表中最匹配的结果**插入指令；Tab 须在「指令」
- **插入后立即校验顺序**（`ax-verify.md` §插入后顺序校验）：「打开网页」必须在「输入文本」之前
...
| 开始 → **输入文本** → **打开网页** → 结束 | ❌ 错误：选中「输入文本」→ 右击 **剪切** → 选中「打开网页」→ **Enter** 创建空行 → **粘贴**（见 `platform-ops.md` §2.3a，不必先删除） |
...
4. **保存前校验**（见 `platform-ops.md` §2.4，**必做、不可跳过**）：
   ...
   - 所有带 `*` 的必填项已填写且**不为空**
   - 弹框内**无任何必填输入框仍带红色边框**
   - 元素选择器：粘贴 XPath 等 1s → 点「为定位器」→ 无匹配时点 close icon →
...
| 现象 | 处理 |
|------|------|
| 必填项输入框红色边框 / 「该字段是必填字段」 | 补全字段；XPath：关「未找到匹配结果」→ 等 1s → 点「为定位器」（`element-selector.md` C1） |
...
4. 全部匹配 → 方可进入「调试 → 运行」
5. 任一不匹配 → 按 `platform-ops.md` §2.3a **剪切→粘贴**（首选）；剪切失败再删后重插 → 再终检
```
...
2. 在**原位置**（结束节点上方、保持顺序）重新 `/` → 浮层搜索框输入中文指令名 → 点击匹配项插入同类型指令
