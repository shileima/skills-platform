...
8. 重点保留以下类型：
   ...
   - 工作相关群聊高未读数
   - HR / 打卡提醒
7. 组织为 Markdown 中文摘要：`未读概览` + 扁平未读列表（不分组、不分类）。每个未读会话一项，使用 `- **会话名**：N条未读` 标题行；标题行后必须空一行；会话内最近 N 条消息必须各自使用二级列表 `  - ` 逐条列举，不能用 `；` 压成一长段；单条消息截断至约 120 字，但不得因会话总长度限制而只保留最后一条；多个会话之间插入空行 + `------------------------------------------------` + 空行；每条会话来源（群名称、系统会话名）必须加粗；同一会话只保留一项，不重复展示。
...
- 每个会话标题后必须保留一个空行；会话内消息必须用缩进二级列表逐条列出，例如 `  - 第1条消息`，禁止把多条消息用分号拼成一行。
