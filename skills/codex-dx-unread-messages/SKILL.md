...
description: >
  ...
  提取重点消息、告警、审批和群聊未读内容，先按主题智能概括为「✨ 智能概括」区块，再整理为未读 Markdown 明细，
  并发送到用户本人的大象单聊会话。
...
8. 重点保留以下类型：
   ...
   - HR / 打卡提醒
   - 自动创建上线计划、迭代交付与部署通知
9. **智能概括**：基于全部已采集未读消息，按主题归类生成 `✨ 智能概括` 区块（见下方格式规范），再与未读明细合并发送；归类示例：上线计划、迭代交付、故障告警、PR、Talos 发布、审批待办等。
10. 组织为 Markdown 中文摘要：`✨ 智能概括` + `未读概览` + 各会话未读明细。智能概括使用编号主题 + `**[标签]**` 条目；未读明细保持扁平会话列表（不分组）。每个未读会话：标题行使用**无 bullet 的加粗行** `**会话名**：N条未读`（不要在标题前加 `- `，否则消息级列表会被渲染成嵌套的实心黑块）；标题行后必须空一行；会话内最近 N 条消息各自使用顶层 `- ` 列表逐条列举，不能用 `；` 压成一长段；群聊消息必须以 `- **发送人**：正文` 的形式给出，把发送人加粗；单聊无发送人时直接 `- 正文`；单条消息截断至约 120 字；多个区块/会话之间插入空行 + `------------------------------------------------` + 空行；同一会话只保留一项。
...
发送内容必须使用如下结构：先 **智能概括**（主题归类 + 标签条目），再 **未读概览** 与各会话明细。
...
1. 上线计划创建与值班QA安排
- **[自动创建上线计划]** 系统于2026-08-06 08:00自动创建ED/FSD日常迭代--20260807上线计划，值班QA为@刘均炳，预计上线时间2026-08-07 22:00:00
- 与我有关：无

2. 点餐营销讨论
- **[话题引入]** @王金财 询问"点餐营销"相关内容，引发群内讨论。
- 与我有关：无

3. OCR接口变更与迁移
- **[接口下线]** @杨丁 指出原 OCR 接口已下线，需统一走 newapi，并提供大模型调用接口文档
- **[后续跟进]** @刘晓垚 反馈官方接口仍报错，@刘永刚 转给 @马世磊 处理
- 与我有关：被 @马世磊 提及；需你跟进处理

------------------------------------------------

未读概览：未读标记：未读 15、未读 5；会话未读：5条、57条、12条

**08.03-发布群-Automan**：5条未读

- **Talos Robot**：Automan 开始发布
- **Talos Robot**：发布成功，环境 staging
...
- feat: Enhance brand configuration handling with alias fallback
- bugfix-cj-0717 -> master

------------------------------------------------

**P3故障**：1条未读

- com.sankuai.fst.qa.fe 504 告警，持续 47 分钟
```

### 智能概括规则

- 标题固定为 `✨ 智能概括`，与下方未读明细之间用 `------------------------------------------------` 分隔。
- 按业务主题编号归类（如上线计划、迭代交付、故障告警、PR、Talos 发布、审批待办）；同一主题下合并相关消息。
- 每条概括使用 `- **[标签]** 正文` 格式；标签取自消息类型（如 `自动创建上线计划`、`话题引入`、`工具使用观点`、`Talos 发布`）。
...
- 每个会话标题（`**会话名**：N条未读`）不加 `- ` 前缀；标题后必须保留一个空行；会话内消息必须用顶层 `- ` 列表逐条列出，禁止使用两个空格缩进的二级列表（会被渲染成实心黑块），也禁止把多条消息用分号拼成一行。
