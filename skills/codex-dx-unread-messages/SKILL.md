...
8. 重点保留以下类型：
   ...
   - 工作相关群聊高未读数
   - HR / 打卡提醒
7. 组织为 Markdown 中文摘要：`未读概览` + 扁平未读列表（不分组、不分类）。每个未读会话：标题行使用**无 bullet 的加粗行** `**会话名**：N条未读`（不要在标题前加 `- `，否则消息级列表会被渲染成嵌套的实心黑块）；标题行后必须空一行；会话内最近 N 条消息各自使用顶层 `- ` 列表逐条列举，不能用 `；` 压成一长段；群聊消息必须以 `- **发送人**：正文` 的形式给出，把发送人加粗；单聊无发送人时直接 `- 正文`；单条消息截断至约 120 字；多个会话之间插入空行 + `------------------------------------------------` + 空行；同一会话只保留一项。
8. **邀请类系统消息一律过滤**：形如 `xx邀请yy加入了群聊`、`新成员入群可查看所有的历史消息`、`xx加入了群聊` 的消息不计入摘要，不占未读名额。
