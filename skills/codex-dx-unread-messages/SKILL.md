...
description: >
  ...
  提取重点消息、告警、审批和群聊未读内容，按主题智能概括为「✨ 智能概括」区块（每个主题标题标注未读条数，并逐条列出未读消息），
  并发送到用户本人的大象单聊会话。
...
7. **逐会话下钻**：对每个 N > 0 的可见未读会话（按当前可见列表尽量全部处理）：
...
8. 重点保留以下类型：
   ...
   - HR / 打卡提醒
   - 自动创建上线计划、迭代交付与部署通知
9. **智能概括**：基于全部已采集未读消息，按主题/会话归类生成唯一的 `✨ 智能概括` 区块（见下方格式规范，每个主题标题标注未读条数并逐条列出未读消息）；归类示例：上线计划、迭代交付、故障告警、PR、Talos 发布、审批待办等。
10. 组织为 Markdown 中文摘要：仅 `✨ 智能概括` 一个区块，**不再输出「未读概览」及各会话未读明细模块**（避免消息重复）。智能概括使用编号主题，每个主题标题后直接标注该主题的未读条数，格式为 `N. 主题名 X条未读`；主题内先用 `- **[标签]** 正文` 概括，再逐条列出该主题下的未读消息（群聊消息 `- **发送人**：正文`，单聊无发送人时 `- 正文`），单条消息截断至约 120 字；每个主题末尾追加 `- 与我有关：…`；多个主题之间插入空行 + `------------------------------------------------` + 空行。
11. **邀请类系统消息一律过滤**：形如 `xx邀请yy加入了群聊`、`新成员入群可查看所有的历史消息`、`xx加入了群聊` 的消息不计入摘要，不占未读名额。
12. 摘要生成前必须去噪：过滤历史 `大象消息汇总`、`智能概括`、AX 类型前缀（`文本`/`text`/`container`）、纯时间、纯数字、导航项和过长链接，避免旧摘要被再次吸入。
13. 调用公共模块 `dx-send-markdown` 发送摘要（`skill.json` 声明 `"modules": ["dx-send-markdown"]`，安装后在 `modules/dx-send-markdown/scripts/send-markdown.sh`）：
...
```js
{
  ...
  if (!markdownButtonLine) {
    ...
    if (!inputLine) {
      ...
      const hasContent = /大象消息汇总|智能概括/.test(filledState.text);
...
发送内容只保留 **智能概括** 一个区块（主题归类 + 标签条目 + 逐条未读）。**不再输出「未读概览」及各会话未读明细**，避免同一条消息重复展示。每个主题标题后直接标注该主题的未读条数。
...
1. 职场清风侠(Claire) 1条未读
- **[提醒]** 到饭点儿啦～中午了该去填填肚子了，别饿着自己呀，吃饱了下午才有力气干活
- 与我有关：无

2. 团团❤️暖心 1条未读
- **[活动通知]** ❤️第148期暖心课堂-【七夕特辑】积极心理讲座来啦，涵盖身心健康、暖心祝福、暖心地图
- 与我有关：无

3. GUI agent 3条未读
- **[讨论]** @杜成林 反馈工具执行完了但对话还没结束，页面状态已结束，仍有小问题
- **杜成林**：李栋焱
- **杜成林**：[图片]
- **杜成林**：还是有点小问题，工具执行完了，但是对话还没完，页面状态是结束了
- 与我有关：无

4. 外卖UI自动化问题反馈 1条未读
- **[问题反馈]** @朱晓燕 能触发重试，但前置设备占用失败，导致重试失败
- **朱晓燕**：能触发重试，但是前置设备占用失败，所以重试失败了
- 与我有关：无
```

### 智能概括规则

- 标题固定为 `✨ 智能概括`，整个摘要只保留这一个区块，不再有「未读概览」及会话明细模块。
- 按业务主题/会话编号归类（如上线计划、迭代交付、故障告警、PR、Talos 发布、审批待办，或直接用会话名作为主题）；同一主题下合并相关消息。
- 每个主题标题必须标注未读条数，格式为 `N. 主题名 X条未读`（X 为该主题/会话的未读数）。
- 主题内先用 `- **[标签]** 正文` 概括（标签取自消息类型，如 `自动创建上线计划`、`话题引入`、`提醒`、`Talos 发布`），再逐条列出该主题下的未读消息：群聊消息 `- **发送人**：正文`，单聊无发送人时 `- 正文`，单条截断至约 120 字。
- 正文中必须保留并体现 `@某人`：群聊发送人前缀 `@发送人`；消息里已有的 `@提及` 原样保留；结构化字段中的人名（如值班 QA、申请人）转为 `@姓名`。
- 每个主题末尾追加 `- 与我有关：…`，需**智能判断**该讨论/内容是否与本人（接收人）相关，并给出具体原因，例如：
  - 被 `@你` / `@本人姓名` 直接提及；
  - 正文提及本人姓名；
  - 本人是本次值班 QA；
  - 待你审批、转交给你处理、请你协助、需你跟进等指向本人的动作。
  - 命中多个原因用 `；` 连接；确无关联时写 `无`。
- 概括正文保留关键时间、人员、项目/迭代名，单条不超过约 200 字；避免把多个来源揉成一条长句。
- 无法归类时可按会话名作为主题，但仍需 `[标签]` 前缀。
...
- 不把历史摘要内容（含 `智能概括`）再次纳入本次摘要。
- 不输出纯时间、纯数字、导航项。
- 链接只保留为“链接”或直接省略。
- 每个主题/会话只作为一个主题出现一次；按当前可见未读会话尽量全部处理，不再限制为 10 个。
- 主题标题（`N. 主题名 X条未读`）后直接接 `- ` 列表项；消息必须用顶层 `- ` 列表逐条列出，禁止使用两个空格缩进的二级列表（会被渲染成实心黑块），也禁止把多条消息用分号拼成一行。
- 群聊消息必须给发送人加粗：`- **发送人**：正文`；单聊消息无发送人时直出正文：`- 正文`。
- 邀请类系统消息（`xx邀请yy加入了群聊`、`新成员入群可查看所有的历史消息` 等）一律过滤，不出现在摘要中。
