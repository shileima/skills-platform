...
**满足 1–6 → 同一轮 automation 内连续执行**：`axButtonIdx(lines, "调试")` 点顶部「调试」→ 确认出现「随机设备」调试弹框（若弹出「运行配置」则 Escape 关掉重试）→ 弹框内用 `debugPanelRunIdx` 点橙色「运行」（默认随机设备）→ 等待执行 → 四处扫描。禁止在步骤 5 与调试之间插入用户交互。

> 🚫 **禁止** `axButtonIdx("运行")` 启动调试。顶部「运行」右侧无名齿轮会打开「运行配置」（重置 / 仅保存 / 保存并运行），不是调试。找不到「检查」时也禁止点无名按钮。
