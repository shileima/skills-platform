...
> 所有场景均遵循 [test-workflow.md](../test-workflow.md)：**配置阶段禁止点「调试」**（失焦用「编排区」Tab + Escape）；全部指令保存 → 一次「检查」→ **一次性**「调试 → 运行」。B 站默认场景实测路径见 [bilibili.md](bilibili.md) §实测黄金路径。
...
```
逐条：插入 + 配表 + 保存 + canvas 验证（禁止调试）
→ 全部完成后：检查 + 顺序终检
→ 一次性：调试 → 运行 → 四处扫描
```

元素选择器：**默认 LLM 动态定位**（§LLM 自然语言描述写法：位置 + 目标元素 + 操作意图）；LLM 失败或运行报错后再新建 Tab 批量采集 XPath。
