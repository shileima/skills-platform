...
2. **新建一个工作流**（点「**可视化编排**」），得到仅含「开始节点」「结束节点」的空工作流（见 `platform-ops.md` §2.1c）
3. 在**编排区**按测试场景规定的**顺序**插入待测指令（不可打乱顺序）
4. 每条指令：**完善表单配置** → **保存** → **读 canvas 节点摘要验证**；保存时报错则当场修复
5. **全部指令保存后**点一次「**检查**」；**配置阶段禁止点「调试」**（见下节 §配置阶段 vs 调试阶段）
6. 终检通过 → **一次性**「调试 → 运行」→ 四处扫描
7. 根据报错修复；修复后可再一次性调试；禁止边插边调

## 配置阶段 vs 调试阶段

| 阶段 | 做什么 | 禁止 |
|------|--------|------|
| 配置 | 逐条插入 → 配表 → 保存 → **canvas 摘要验证** | 点「调试」、用「调试」失焦 canvas、等云浏览器跑完 |
| 终检 | 一次「检查」+ 顺序终检 + 配置 icon 终检 | 调试 |
| 调试 | **仅 1 次**「调试 → 运行」（修复失败项后可再调） | 每插一条就调试 |

## 完整流程

```
新建空工作流
  → 逐条：插入 + 配表 + 保存 + canvas 摘要验证（禁止点调试）
  → 全部完成后：检查 + 终检
  → 一次性：调试 → 运行
  → 四处扫描 → 修复 → （必要时）再一次性调试
```
...
1. 点击「**+ 新建工作流**」→ 点选「**可视化编排**」→ 填写名称 →「**创建工作流**」
2. 确认编排区仅有 **开始节点** 和 **结束节点**

> sky 脚本见 `platform-ops.md` §2.1c。旧文案「编排模式」= 现「可视化编排」。
...
- 在右侧「指令」Tab 搜索框（placeholder「请输入」）**set_value 中文指令名**，如「打开网页」「输入文本」「点击」「刷新网页」
- **双击**「网页自动化」分组下匹配的结果——多数为 `xxx (web)`；**「刷新网页」常无 `(web)` 后缀**（见 `insert-command.md` §「刷新网页」搜索结果 AX 差异）
- **锚点**：向后追加时选中**上一条已保存指令**（用 canvas 摘要关键词定位，如 `bilibili.com`），再 Enter → 搜索 → 双击
...
| 场景表（bilibili.md） | canvas 摘要应有顺序 |
|----------------------|---------------------|
| 1 打开网页 | `打开网页 url: https://www.bilibili.com` |
| 2 输入文本 | `元素中输入 bilibili` |
| 3 点击 | `点击 页面`（**非** `selectorId`） |
| 4 刷新网页 | `刷新网页` |

**通过示例**：`开始 → 打开网页 url:… → 元素中输入 bilibili → 点击 页面 → 刷新网页 → 结束` ✅
