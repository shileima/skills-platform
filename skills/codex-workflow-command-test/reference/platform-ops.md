...
配置页顶栏大致为：`创建技能 | 调试 | 运行 | 无名齿轮 | 日志 | 变量 | 更多`。

| 目标 | 怎么点 | 禁止 |
|------|--------|------|
| 开始调试 | **只** `axButtonIdx(lines, "调试")` | `axButtonIdx("运行")`；点「运行」右侧无名按钮 |
| 调试弹框确认 | `debugPanelRunIdx`（「随机设备」切片内名称精确为「运行」） | 把工具栏「运行」当调试确认 |
| 「检查」 | 有「检查」文案才点 | 本页若无「检查」，**不要**用无名按钮降级 |

误开「运行配置」（标题含 hook 事件 / 按钮为「重置」「仅保存」「保存并运行」）→ `Escape` 关掉，再点「调试」。
