...
当 `test-workflow.md` §调试前配置终检 与 §调试前场景顺序终检 均 `readyForDebug: true`，且保存/检查无报错时：

- **立即**执行本节「调试运行」，**禁止**询问用户是否运行
- **禁止**输出半成品状态后等待确认；应修复阻塞项或在同一 automation 轮次内点「调试 → 运行」
- 调试脚本见下方；终检与调试可合并为**连续 sky exec**（先 `pre-debug-config-check`，`readyForDebug` 则紧接着 `debug-run`）
...
1. 选中该指令节点 → `Delete` 删除
2. 在**原顺序位置**（选中锚点指令 → Enter 空行）重新走搜索+双击流程（右侧「指令」Tab 搜索框 → 双击「网页自动化」分组下 `xxx (web)` 结果）
