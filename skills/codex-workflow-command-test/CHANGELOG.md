...
- **调试按钮误点「运行配置」**：`axButtonIdx` 改为按钮无障碍名称精确全等；启动调试只用 `axButtonIdx("调试")`，禁止 `axButtonIdx("运行")`。顶部「运行」右侧无名齿轮会打开「运行配置」，点偏后 Escape 关闭并重试。「运行」仅在调试弹框「随机设备」切片内由 `debugPanelRunIdx` 点击。

## [0.0.11] - 2026-08-24

### 变更

- **运行时整理**：统一通过 `exec.sh -f` 在 `/exec` 隔离作用域执行，直接使用运行时注入的 `sky.*` / `ax.*`
- 新增 `scripts/workflow-runtime.mjs`、`scripts/run-workflow-hosted.sh`、`scripts/ensure-ready.sh`、`scripts/debug-run-once.sh`
- **`reference/sky-runtime.md`** / **`reference/prerequisites.md`** / **`SKILL.md`**：对齐 cua-router-basic 0.4.18 运行规范
- 文档步骤统一使用 `emitResult`；就绪探针改为依赖自带的 `scripts/ensure-ready.sh`

## [0.0.10] - 2026-08-22

### 新增

- **`reference/sky-runtime.md`**：Sky 运行时共享模块（helper 包、`/exec` 步骤批次策略、Chrome 前台、`defocusCanvas`、插入/LLM/XPath/保存/调试脚本、B 站四步黄金路径、cgWindowNotFound 恢复）
